...
public class LectureServiceGetPopularListTest {
    @InjectMocks
    ...
    public void lectureList_Success() {
        ...
        Page<Lecture> lecturePage = new PageImpl<>(lectureList, PageRequest.of(page, size), lectureList.size());

        Mockito.when(lectureRepository.findPopularLectures(periodDays, PageRequest.of(page, size)))
                .thenReturn(lecturePage);

        // When
        LectureListResponse response = lectureService.getPopularLectureListByApplicationCount(page, size, periodDays);
